...
import { UnifiedTagTable } from '@/components/admin/UnifiedTagTable';
...
// Define Tag type locally or import if available
interface Tag {
    id: number;
    name: string;
...
function TagsContent() {
    ...
    // Sort states
    const [sortColumn, setSortColumn] = useState(searchParams.get('sort_by') || 'recipes_count');
    ...
    const fetchTags = useCallback(async () => {
        ...
        try {
            ...
            const response = await api.admin.getAdminTags(token, {
                page,
                per_page: perPage,
                search: searchQuery,
                sort_by: sortColumn,
                sort_dir: sortDirection
            }) as { tags: Tag[], pagination: Pagination };
    ...
    if (user && user.role !== 'admin' && user.role !== 'moderator') {
        // Moderators can manage tags per API routes usually, but let's check role access in task requirements
        // "Tags Dashboard accessible by admin and moderator"
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h2 className="text-2xl font-bold tracking-tight">إدارة الوسوم</h2>
